...
// ternery = '?' expr | primary ':' expr | primary
func parseTernery(lex *lexer) Expr {
	x := parseExpr(lex)
	if lex.token == '?' {
		op1 := lex.token
		y := parseExpr(lex)
...
func parsePrimary(lex *lexer) Expr {
	...
	case scanner.Ident:
		...
		if lex.token != closeToken {
			...
			if lex.token != closeToken {
				msg := fmt.Sprintf("got %q, want %q", lex.token, closeToken)
				panic(lexPanic(msg))
			}
		}
		lex.next() // consume ')'
